...
def main(args):
    ...
    if MODE == 'train':
        ...
        for epoch in range(EPOCHS):
            ...
            print(f'epoch {epoch + 1}, train loss {round(train_mse_loss, 4)} - {round(train_mae_loss, 4)}, valid loss {round(valid_mse_loss, 4)} - {round(valid_mae_loss, 4)} \n')
            ...
            if NUM_TRIALS > 0:
                if not early_stopper.is_continuable(model, valid_mae_loss, losses=(train_mae_loss, valid_mae_loss, None), epoch=epoch+1):
                    print('early stop. \n')
        ...
        infer_data[['Class', 'mse', 'mae']].to_csv(INFER_OUTPUT_PATH + f"{infer_model_params['model_name']}_error.csv", index=False)

    elif MODE == 'test':
        import matplotlib.pyplot as plt
        from sklearn.metrics import roc_curve, precision_recall_curve, auc
        mse_data = pd.read_csv("/home/mi/PycharmProjects/ml-mining/files/ae/2021-01-10 12:27:09_AE.pt_error.csv")
...
if __name__ == "__main__":
    ...
    # TODO 使用新修改的时间差文件打分
    parser.add_argument('--infer_data_path', type=str, default="/home/mi/PycharmProjects/ml-mining/data/credit_card_valid.csv")

...
# ver 0.1.6
# default woe replcement for labels not observable in training dataset added in transform method
...
class WoE(object):
    ...
    def __init__(self, qnt_num=16, min_block_size=16, spec_values=None, v_type='c', bins=None, t_type='b'):
        ...
        :t_type : Binary 'b' or continuous 'c' target variable
    ...
    def fit(self, x, y):
        ...
        df = pd.DataFrame({"X": x, "Y": y, 'order': np.arange(x.size)})#将传入的X,y合成df
        # Separating NaN and Special values
        df_sp_values, df_cont = self._split_sample(df) #返回特殊值的df 和 非特殊值的df
        # # labeling data
        df_cont, c_bins = self._cont_labels(df_cont) #对非特殊值的df处理，返回有label的df
        df_sp_values, d_bins = self._disc_labels(df_sp_values)#对特殊值的df处理，返回有label的df
        # getting continuous and discrete values together
        self.df = df_sp_values.append(df_cont) #合并特殊值分箱df，非特殊值分箱df
        self.bins = d_bins.append(c_bins) #合并d_bins c_bins
        # calculating woe and other statistics
        self._calc_stat() #直接调用对df计算woe iv pd local score等值
    ...
    def _split_sample(self, df): #将包含特殊值的df 和 不包含特殊值的df 分开处理
        if self.v_type == 'd':
            return df, None
        sp_values_flag = df['X'].isin(self.spec_values.keys()).values | df['X'].isnull().values
        df_sp_values = df[sp_values_flag].copy()  #存 特殊的值的df
        df_cont = df[np.logical_not(sp_values_flag)].copy() #存 非特殊值的df
        return df_sp_values, df_cont

    def _disc_labels(self, df):#对特殊值处理，加入label "d_"+str() 等
        # print("__disc__labels")
    ...
    def _cont_labels(self, df): #传入 正常的df 排除特殊值的df
        # print("打印：__cont__labels")
        ...
        bins = None
        cuts = None
        if self._predefined_bins is None: #self._predefined_bins为参数bins
            try:
                cuts, bins = pd.qcut(df["X"], self.qnt_num, retbins=True, labels=False) #labels 控制返回标签，F为0,1,2,3，T为区间[A，B]
        ...
        #c_bins df为bin的点labels为‘0‘ ’1‘ ’2‘ ’3‘ ....
    ...
    def _calc_stat(self):
        ...
        #增加pd转化分
        # stat['pd'] = stat['mean'].apply(lambda x:log(x/(1-x)))
        # stat['local_score'] = 100 * (stat['pd'].max() - stat['pd']) / (stat['pd'].max() - stat['pd'].min())

        iv_stat = (stat['good'] / t_good - stat['bad'] / t_bad) * stat['woe']
        self.iv = iv_stat.sum()
        # adding stat data to bins
        #将计算的bins的 woe ops mean 等返回到self.bins属性
        self.bins = pd.merge(stat, self.bins, left_index=True, right_on=['labels'])

        label_woe = self.bins[['woe', 'labels']].drop_duplicates() #去重
        #右连接，以label_woe为准 ，重新返回self.df
        # print("stat:\n",stat)
    ...
    def transform(self, x, manual_woe=None, replace_missing=None):
        ...
        :replace_missing: replace woe for labels not observable in training dataset by this value
        ...
        tr_bins = self.bins[['local_score','woe', 'labels']].copy()
        # tr_bins['pd'] = tr_bins['mean'].apply(lambda x:math.log(x/(1-x)))
        # tr_bins['local'] = 100 * (tr_bins['pd'].max() - tr_bins['pd'])/(tr_bins['pd'].max() - tr_bins['pd'].min())
        # print('tr_bins:',tr_bins)
        if manual_woe:
        ...
        # print(df.head())
        # assigning woe
        df = pd.merge(df, tr_bins[['local_score', 'woe', 'labels']].drop_duplicates(), how='left', left_on=['labels'], right_on=['labels'])
        # print(df.head())
        # returning to original observation order
    ...
    def merge(self, label1, label2=None):
        ...
        In case of continuous variable, only label1 should be provided. It will be merged with the next label.
    ...
    def plot(self, sort_values=True, labels=False):
        ...
        if self.v_type != 'd':
            ...
            # plot_data = pd.concat([plot_data[~cont_labels], temp_data])
            cont_labels = plot_data['labels'].apply(lambda z: not z.startswith('d_'))
        ...
        if sort_values:
            if self.v_type != 'd':
                ...
                plot_data = pd.concat([plot_data[~cont_labels].sort_values('labels'), temp_data])
            else:
        ...
        plt.bar(index, plot_data['obs'], bar_width, color='steelblue', label='Observations')
        ax2 = ax.twinx()
        ax2.set_ylabel('Weight of Evidence')
        ax2.plot(index, plot_data['woe'], 'bo-', linewidth=4.0, color='orange', label='WoE')
    ...
    def optimize(self, criterion=None, fix_depth=None, max_depth=None, cv=3, scoring=None, min_samples_leaf=None):
        ...
        # print(final_tree.tree_.threshold)
...
if __name__ == "__main__":
    ...
    if t_type_ == 'b':
        y_ = np.where(np.random.rand(N, ) + x1 + x2 > 1, 1, 0)
    else:
